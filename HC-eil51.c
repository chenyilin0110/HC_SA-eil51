...
int main()
{
	...
	double bestDistance = 0,startDistance = 0,foreveryzero= 0,iterationData[31][5000]={0};
	...
	{		
		...
		while(co != 50001)
		{
			...
			if(co%10 == 0)
			{
				iterationData[iteration-1][w-1] = bestDistance;										
				w++;
			}
		}		
		for(i=0;i<count;i++)
	...
	for(j=0;j<w;j++)	
		for(i=0;i<iteration;i++)
			iterationData[30][j] += iterationData[i][j];
	for(j=0;j<w;j++)			
		iterationData[30][j] = iterationData[30][j] / iteration;
	for(j=0;j<w-1;j++)			
		printf("%d %f\n",j+1,iterationData[30][j]);
	endclock = clock();	
